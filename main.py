...
SPREADSHEET_NAME        = "amazon_sourcing_history"
WORKSHEET_NAME          = "history"

# ---------------------------
# 소싱 조건 설정
# ---------------------------
REVIEW_SPIKE_THRESHOLD = 50   # 7일 내 리뷰 증가량 기준
NAVER_MAX_COUNT        = 0    # 네이버 검색결과 허용 최대 수 (0 = 완전 미등록)
CRAWL_DELAY_MIN        = 2
CRAWL_DELAY_MAX        = 5
...
# ============================================================
# Google Sheets 히스토리 관리 함수 (기존과 동일)
# ============================================================
...
def inject_test_history(today_products: list) -> pd.DataFrame:
    """테스트 모드용 가짜 히스토리 생성 (TEST_MODE=True일 때만 사용)"""
...
def extract_search_keywords(title_ja: str) -> tuple | None:
    ...
    if not words:
        return None
    brand_keyword = words[0]
    core_keyword  = " ".join(words[1:4]) if len(words) > 1 else words[0]
    return brand_keyword, core_keyword

# ============================================================
# Amazon JP 크롤링 (51~100위 수집)
# ============================================================
CRAWL_HEADERS = {
...
def crawl_category(category: str, url: str) -> list:
    ...
    try:
        ...
        # 51위부터 100위까지만 수집 (최대 50개)
        for rank, item in enumerate(items[50:100], start=51):
            title_el = (
...
def build_email_body(products: list, is_test: bool) -> str:
    ...
    <p style="color:#666;">{today} 기준 | 리뷰 급증(+{REVIEW_SPIKE_THRESHOLD}개↑) + 네이버 미등록 상품 (51~100위)</p>
    <hr>
    """
    ...
    body += """
    <hr>
    <p style="color:#999;font-size:12px;">본 메일은 자동발송됩니다.</p>
    </body></html>
    """
    return body
...
def main():
    ...
    if not spiked_products:
        log.info("리뷰 급증 상품 없음.")
        # 급증 상품이 없으면 이후 단계 생략 (이메일 미발송)
        return
    ...
    # 8. 요일 체크: 월요일(weekday=0)에만 이메일 발송 (운영 모드)
    if not TEST_MODE:
        today_weekday = datetime.now().weekday()  # 월요일=0, 일요일=6
        if today_weekday != 0:
            log.info("오늘은 월요일이 아니라서 이메일을 발송하지 않습니다. (결과는 CSV로만 저장)")
            return

    # 9. 이메일 발송 (테스트 모드이거나 월요일인 경우)
